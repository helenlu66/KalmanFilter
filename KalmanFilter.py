...
if __name__ == "__main__":
    ...
    mu_prev = mu_t_minus_1
    sigma_prev = sigma_t0
    u_curr = u_motor_commands
    filter = KalmanFilter(A=A, B=B_motor_commands, R=R, C=C, Q=Q)
    with open('state_prediction.txt', 'w') as file:
        means = filter.mu_prediction(mu_prev=mu_prev, u_curr=u_curr)
        cov = filter.calculate_covariance(sigma_prev=sigma_prev)
        file.write(f"u matrix:\n{u_curr}\n")
        file.write(f"predicted state:\n{means}\n")
